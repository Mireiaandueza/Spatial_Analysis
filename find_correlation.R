...
filtered <- eval(parse(text = paste0("subset(x = ST, subset = ", feature1,"> 0)")))
filtered <- eval(parse(text = paste0("subset(x = filtered, subset = ", feature2,"> 0)")))
...
cor_test_func <- function(data) {
	...
	exp_feature2 <- FetchData(data, vars = feature2)[, 1]
  	...
  	return(c(Pearson_Correlation = cor_result$estimate, P_Value = cor_result$p.value))
}
...
plot2 <- FeatureScatter(filtered, feature1 = feature1, feature = feature2, group.by = "Genotype",  cols = c("darkorchid4", "mediumorchid1","grey")) +
xlab(feature1) + ylab(feature2) + ggtitle(paste0("WT:", round(results[1,2],2),"  DD:", round(results[1,1],2),"  HET:",round(results[1,3],2)," Total:", round(total_cor_result$estimate,2))) 
print(plot2)
dev.off()
}
...
spatial_features <- function(ST, slide, feature1,feature2, out_dir){
	...
	plot1 <- SpatialFeaturePlot(ST_sub, features = feature1, images = slide, pt.size.factor = 1.8, image.alpha = 0) + theme(legend.position = "top") +
      	scale_fill_gradientn(colors = c( "blue","yellow","red"),
                   limits = c(scale_range1[1],scale_range1[2]), values = rescale(c(scale_range1[1],(scale_range1[1]+scale_range1[2])/2,scale_range1[2])))
	plot2 <- SpatialFeaturePlot(ST_sub, features = feature2, images = slide, pt.size.factor = 1.8, image.alpha = 0) + theme(legend.position = "top") +
      	scale_fill_gradientn(colors = c( "blue","yellow","red"),
                   limits = c(scale_range2[1],scale_range2[2]), values = rescale(c(scale_range2[1],(scale_range2[1]+scale_range2[2])/2,scale_range2[2])))
